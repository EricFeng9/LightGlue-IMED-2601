...
class SuperPointLoRATrainer(pl.LightningModule):
    ...
    def sample_descriptors_at_keypoints(self, dense_desc, keypoints):
        """在关键点位置采样描述子"""
        if len(keypoints) == 0:
            return torch.empty(0, dense_desc.shape[1], device=dense_desc.device)
        
        B, D, H, W = dense_desc.shape
        
        kpts_scaled = keypoints.clone()
        kpts_scaled = kpts_scaled / 8.0 - 0.5
        
        kpts_normalized = kpts_scaled.clone()
        kpts_normalized[:, 0] = (kpts_normalized[:, 0] / (W - 1)) * 2 - 1
        kpts_normalized[:, 1] = (kpts_normalized[:, 1] / (H - 1)) * 2 - 1
    ...
    def merge_and_deduplicate_keypoints(self, kpts_real, kpts_fake):
        """聚合并去重关键点"""
        if len(kpts_real) == 0 and len(kpts_fake) == 0:
            return torch.empty(0, 2, device=kpts_real.device)
        ...
        all_kpts = torch.cat([kpts_real, kpts_fake], dim=0)
        
        merged_kpts = []
        used_mask = torch.zeros(len(all_kpts), dtype=torch.bool, device=all_kpts.device)
        
        for i in range(len(all_kpts)):
            if used_mask[i]:
                continue
            
            dist = torch.norm(all_kpts - all_kpts[i:i+1], dim=1)
            close_mask = dist < self.merge_threshold
            
            used_mask |= close_mask
            
            merged_kpt = all_kpts[close_mask].mean(dim=0)
            merged_kpts.append(merged_kpt)
        
        if len(merged_kpts) == 0:
            return torch.empty(0, 2, device=kpts_real.device)
        
        return torch.stack(merged_kpts, dim=0)
