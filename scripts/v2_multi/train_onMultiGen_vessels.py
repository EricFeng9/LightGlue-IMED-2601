...
class GenDatasetWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 构建返回结果
        result = {
            'image0': data['image0'],                  # [1, H, W] fix (固定图)
            'image1': data['image1'],                  # [1, H, W] moving (形变)
            'image1_gt': data['image1_gt'],            # [1, H, W] moving (原始未形变)
...
class MultimodalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            logger.info(f"训练集加载 260305_1_v30 生成数据: {len(self.train_dataset)} 样本")
