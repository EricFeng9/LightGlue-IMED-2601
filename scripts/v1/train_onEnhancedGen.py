...
class GenDatasetWrapper(torch.utils.data.Dataset):
    def __init__(self, base_dataset, apply_domain_randomization=True, augment_prob=0.5):
    ...
    def __getitem__(self, idx):
        ...
        # 【域随机化增强】50%概率对每张图像独立应用域随机化
        if self.apply_domain_randomization:
            if random.random() < self.augment_prob:
                image0 = random_domain_augment_image(image0)
            if random.random() < self.augment_prob:
                image1 = random_domain_augment_image(image1)
        
        # 直接使用新数据集的输出，添加 image1_gt 字段
        # 由于新数据集中 CF 和原始 FA 是对齐的，我们用 image0 作为 image1_gt 的参考
        result = {
            'image0': image0,                  # [1, H, W] CF (固定图，可能已增强)
            'image1': image1,                  # [1, H, W] FA deformed (未配准的移动图，可能已增强)
...
class MultimodalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.train_dataset = GenDatasetWrapper(train_base, apply_domain_randomization=True, augment_prob=0.5)
...
def main():
    ...
    logger.info("早停配置: monitor=combined_auc, start_epoch=0, patience=15, min_delta=0.0001")
    ...
    logger.info(f"开始混合训练 (训练集: 260227_2_v29_2_1 生成数据 | 验证集: operation_pre_filtered_cffa 真实数据): {args.name}")
