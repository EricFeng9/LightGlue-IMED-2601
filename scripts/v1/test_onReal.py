...
class MultimodalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            # 训练集使用 operation_pre_filtered_cffa，验证集使用 CFFA
            ...
            train_base = CFFADataset_Train(root_dir=str(train_data_dir), split='train', mode='fa2cf')
            val_base = CFFADataset_Val(root_dir=str(val_data_dir), split='val', mode='fa2cf')
...
class PL_LightGlue_Real(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        """训练步骤（真实数据训练，不使用 Domain Randomization）"""
        outputs = self(batch)
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="LightGlue CFFA Real-Data Training")
    parser.add_argument('--name', '-n', type=str, default='lightglue_cffa_baseline', help='训练名称')
