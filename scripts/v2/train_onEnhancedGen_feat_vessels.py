...
class GenDatasetWrapper(torch.utils.data.Dataset):
    def __init__(self, base_dataset, apply_domain_randomization=False):
        ...
        # 注意：不再使用图像级域随机化，改用特征级域随机化
        self.apply_domain_randomization = False  # 强制禁用
    ...
    def __getitem__(self, idx):
        ...
        # 【关键修改】不再应用图像级域随机化
        # 保持图像干净，确保 SuperPoint 提取的关键点位置准确
        # 域随机化将在特征级别进行（见 forward 函数）
...
class MultimodalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.train_dataset = GenDatasetWrapper(train_base, apply_domain_randomization=False)  # 禁用图像级域随机化
...
class PL_LightGlue_Gen(pl.LightningModule):
    ...
    def __init__(self, config, result_dir=None):
        ...
        # 特征级域随机化参数
        self.feature_noise_std = 0.1      # 高斯噪声标准差
        self.feature_dropout_rate = 0.1   # 特征维度 Dropout 比例
    ...
    def forward(self, batch):
        ...
        # ==========================================
        # 为什么有效？
        # 1. 保护几何锚点：图像干净 -> SuperPoint 提取的关键点位置准确 -> GT 匹配对 100% 正确
        # 2. 模拟跨模态差异：在描述子上注入噪声 -> 人为制造"表观差异" -> 逼迫模型学习跨模态不变性
        # 3. 激活上下文推理：Feature Dropout -> 单点特征不可靠 -> LightGlue 必须利用上下文（血管拓扑）
        # ==========================================
        if self.training:
            # 策略 1: 注入适度高斯噪声（模拟模态间的特征差异）
            # 只对 Moving 图（FA）的描述子加噪声，保持 Fixed 图（CF）作为稳定锚点
            noise = torch.randn_like(batch['descriptors1']) * self.feature_noise_std
            batch['descriptors1'] = batch['descriptors1'] + noise
            
            # 重新 L2 归一化（SuperPoint 特征期望在单位超球面上）
            batch['descriptors1'] = torch.nn.functional.normalize(
                batch['descriptors1'], p=2, dim=-1
            )
            
            # 策略 2: 特征 Dropout（强迫 LightGlue 利用上下文）
            # 随机将描述子的某些维度置零，逼迫模型看周围点的拓扑结构
            dropout_mask = (torch.rand_like(batch['descriptors1']) > self.feature_dropout_rate).float()
            batch['descriptors1'] = batch['descriptors1'] * dropout_mask
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="LightGlue Feature-Level Domain Randomization + Vessel-Guided Training")
    parser.add_argument('--name', '-n', type=str, default='260303_feat_level_dr_vessel', help='训练名称')
    ...
    # 特征级域随机化参数
    parser.add_argument('--feature_noise_std', type=float, default=0.05, help='特征噪声标准差 (推荐 0.05-0.15)')
    parser.add_argument('--feature_dropout_rate', type=float, default=0.1, help='特征 Dropout 比例 (推荐 0.1-0.2)')
...
def main():
    ...
    # 应用命令行参数到模型
    model.feature_noise_std = args.feature_noise_std
    model.feature_dropout_rate = args.feature_dropout_rate
    ...
    logger.info("=" * 80)
    logger.info("【特征级域随机化 + 血管引导训练】")
    logger.info("=" * 80)
    logger.info(f"特征噪声标准差: {args.feature_noise_std}")
    logger.info(f"特征 Dropout 比例: {args.feature_dropout_rate}")
    ...
    logger.info("策略: 特征级域随机化（保护几何锚点 + 模拟跨模态差异 + 激活上下文推理）")
