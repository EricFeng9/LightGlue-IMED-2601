...
class OCTFADataset(Dataset):
    """
    OCT-FA 自动配对数据集 - 支持直接从文件夹读取
    支持配准和有效区域筛选
    返回: cond_original, tgt, cond_path, tgt_path
    """
    def __init__(self, root_dir='/data/student/Fengjunming/LoFTR/data/operation_pre_filtered_octfa', split='train', mode='fa2oct'):
        ...
        for subdir in subdirs:
            ...
            # 寻找配对图像 (01 为 OCT, 02 为 FA)
            png_files = glob.glob(os.path.join(subdir_path, "*_01.png"))
            for oct_path in png_files:
                base_name = os.path.basename(oct_path).replace('_01.png', '')
                fa_path = os.path.join(subdir_path, f"{base_name}_02.png")
                oct_pts = os.path.join(subdir_path, f"{base_name}_01.txt")
                fa_pts = os.path.join(subdir_path, f"{base_name}_02.txt")
                
                if os.path.exists(fa_path) and os.path.exists(oct_pts) and os.path.exists(fa_pts):
                    all_samples.append({
                        'fa_path': fa_path,
                        'oct_path': oct_path,
                        'fa_pts': fa_pts,
        ...
        print(f"[OCTFADataset] {split} set: {len(self.samples)} samples from {len(eye_ids) - num_train_eyes if split != 'train' else num_train_eyes} eye images (total {num_total} samples, {num_total_eyes} eye images)")
    ...
    def __getitem__(self, idx):
        ...
        # 3. 确定 fix 和 moving: octfa -> fix=OCT, moving=FA
        fix_np = oct_np
        moving_np = fa_np
        fix_points = oct_points
